...
## Настройка отправки email с промокодами

Для реальной отправки писем с промокодами рекомендуется использовать **EmailJS** - простой сервис для отправки писем из JavaScript без бэкенда.

### Шаг 1: Регистрация в EmailJS

1. Зайдите на [emailjs.com](https://www.emailjs.com/)
2. Создайте бесплатный аккунт
3. Подключите email-сервис (Gmail, Outlook, SendGrid и др.)

### Шаг 2: Создание шаблона письма

В панели EmailJS создайте шаблон со следующими переменными:
...
### Шаг 3: Подключение EmailJS

1. Добавьте скрипт EmailJS в `index.html`:
```html
<script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@3/dist/email.min.js"></script>
```

2. Инициализируйте EmailJS с вашим Public Key:
```javascript
...
В файле `assets/js/main.js` замените mock-функцию `sendPromoCodeEmail` на:
...
async function sendPromoCodeEmail(email, promoCode) {
    const emailData = {
        ...
        subject: `Ваш промокод на 3 бесплатных AI-анализа: ${promoCode}`,
        message: `Ваш промокод: ${promoCode}`
    };
    
    return emailjs.send('YOUR_SERVICE_ID', 'YOUR_TEMPLATE_ID', emailData)
        .then((result) => {
...
1. **Formspree** - простая форма с отправкой на email
2. **Netlify Forms** - если хостинг на Netlify
3. **Собственный API** - для полного контроля

### Текущий функционал

Сейчас форма работает в демо-режиме:
- ✅ Валидация email
- ✅ Генерация уникальных промокодов (формат: AI3FREE + 6 символов)
- ✅ Показ уведомлений пользователю
- ✅ Обработка ошибок
- 🔄 Симуляция отправки email (требует настройки)
